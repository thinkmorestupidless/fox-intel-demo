...
class ApplicationController @Inject() (
  ...
  def bundle(file: String): Action[AnyContent] =
    if (environment.mode == Mode.Dev) Action.async {
      ws.url(s"http://localhost:8080/bundles/$file").get().map { response ⇒
        ...
        val headers = response.headers.toSeq
          .filter(p ⇒ List("Content-Type", "Content-Length").indexOf(p._1) < 0)
